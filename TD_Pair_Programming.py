...
def verify_win(tableau_gagnant, morpion):
    for combinaison in tableau_gagnant:
        ...
        # print(morpion[0,0])
        if morpion[combinaison[0][0], combinaison[0][1]] == morpion[combinaison[1][0], combinaison[1][1]] and morpion[
            combinaison[0][0], combinaison[0][1]] == morpion[combinaison[2][0], combinaison[2][1]]:
...
def tour(morpion, state):
    ...
    while state[-1] == j:
        print("Vous ne pouvez pas jouer 2x d'affiller")
        (j, l, c) = choix_du_joueur(state)
        l = int(l)
        c = int(c)
    state.append(j)
